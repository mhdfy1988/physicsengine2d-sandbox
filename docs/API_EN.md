...
Legacy forwarding headers are also shipped for compatibility:
...
You cannot access internal engine fields directly. Use only APIs exposed in `physics.hpp`.
...
- Preferred include path is now `physics.hpp`, `physics_ext.hpp`, or `physics_world.hpp`.
- Legacy `.h` headers remain available as forwarding shims for downstream compatibility.
- New runtime and tooling code should be compiled as C++.

## 11. Kernel Maintenance Rules

- Implementation changes: `src/*.cpp`
- Public API changes: update the `.hpp` headers and keep the `.h` forwarding headers aligned
...
### 13.4 Collision APIs (`collision.hpp`)
...
### 13.5 Constraint APIs (`constraint.hpp`)
...
### 13.6 Math APIs (`physics_math.hpp`)
