...
1. 工程长期目标是**全工程以 C++ 为主实现**，包括 runtime、editor、asset pipeline、tools 与应用主路径。
2. C 只作为兼容层/桥接层保留在必要边界：
   - 对外公开 C API
   - 少量平台互操作入口
   - 迁移窗口内的临时兼容路径
3. 后续阶段若新增能力，默认直接落在 C++ 实现，不再新增新的 C 主实现模块。
4. 若某处继续保留 C 实现，必须在设计文档中写明：
   - 保留原因
   - 退出条件
   - 计划移除阶段
...
1. 阶段 A-E 已完成，内核 C++ 化、Runtime 固化、内容管线、编辑器主流程与全工程 C++ 收敛基线均已落地。
2. 总设计文档中的剩余未完成项，主要集中在：
   - Project/Workspace/Package 工程体系
   - Prefab 冲突处理与悬挂 Override 修复
   - 插件/扩展机制
   - 构建矩阵、诊断与故障恢复
3. 这些内容不再回填到 A-E，而是在后续阶段继续推进，防止目标漂移。
...
4. A-E 完成后，当前主线任务从“内核 C -> C++ 迁移”转为“工程系统、Prefab 语义、插件扩展与发布工程化能力补齐”。
