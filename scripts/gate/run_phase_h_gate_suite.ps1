...
$results += Invoke-Gate -Name "gate_build_metadata" -FilePath "powershell" -Arguments @("-NoProfile","-ExecutionPolicy","Bypass","-File","scripts/dev/emit_build_metadata.ps1","-Target","editor","-Configuration","gate","-OutputPath","artifacts/build/build_metadata.json")
$results += Invoke-Gate -Name "gate_release_matrix" -FilePath "powershell" -Arguments @("-NoProfile","-ExecutionPolicy","Bypass","-File","scripts/gate/run_phase_h_release_matrix.ps1","-Version","1.0.0","-OutputRoot",(Join-Path $OutputDir "release_matrix"))
